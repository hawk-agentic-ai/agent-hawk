...
async def _handle_view_query(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Accept filters as a JSON string or object for compatibility with Dify manifests
    if isinstance(filters, str):
        ...
        if txt:
            try:
                ...
                # If not valid JSON, fall back to empty or a best-effort single key
                filters = {}
...
# -----------------------------------------------------------------------------
def _build_dify_manifest_json() -> str:
    """Build Dify-compliant manifest as JSON string.
    Note: We emit a raw token for filters so Dify can inject an object directly.
    """
    tools = {
        "tools": [
            {
                ...
                "description": (
                    ...
                    "Use for all pre-trade utilization checks."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "instruction": {
                            "type": "string",
                            "description": (
                                "The user's natural language instruction (e.g., "
                                "'check capacity for hedging 150M HKD in entity 1100')."
                            )
                        },
                        "trace_id": {
                            "type": "string",
            ...
            },
            {
                ...
                "description": "Checks the health and status of the HAWK allocation system.",
                ...
                "endpoint": {
                    ...
                    "payload": {
                        ...
                        "id": "health_check",
                        "method": "tools/call",
                        "params": {"name": "get_allocation_health", "arguments": {}}
                    }
                }
            },
            {
                "name": "hawk_capacity_query",
                "description": "Queries allocation capacity views for specific entities or currencies.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "view_name": {
                            "type": "string",
                            "description": "The view to query (e.g., 'v_entity_capacity_complete')."
                        },
                        "filters": {
                            "type": "object",
                            "description": "JSON object for filters (e.g., {\"currency_code\": \"USD\"})."
                        }
                    },
                    "required": ["view_name"]
                },
    ...
    raw = json.dumps(tools, indent=2)
    return raw

@app.get("/dify/dify-agent-tools.json", include_in_schema=False)
async def get_dify_agent_tools():
    """
    Dify tool manifest (strict JSON) — jq-parseable.
    Filters are declared as a string; server accepts string or object at runtime.
    """
    tools = {
        "tools": [
            {
                "name": "hawk_stage1a_allocation",
                "description": "Processes a natural language instruction for a Stage 1A hedge allocation check. Use for all pre-trade utilization checks.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "instruction": {
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "filters": {
                            "type": "string",
                            "description": "JSON string for filters (e.g., {\"currency_code\": \"USD\"})."
                        }
                    },
                    "required": ["view_name"]
                },
...
async def mcp_discovery(request: Request):
    ...
    # Return a relative URL for portability behind reverse proxies
...
async def mcp_endpoint(request: Request, authorization: Optional[str] = Header(default=None)):
    ...
    try:
        # Parse request first so we can allow unauthenticated handshake methods
        ...
        # Enforce auth only for state-changing or data/tool calls
        requires_auth = method not in ("initialize", "initialized", "notifications/initialized", "tools/list")
        if requires_auth:
