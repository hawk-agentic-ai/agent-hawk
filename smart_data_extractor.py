...
# Import existing hedge management cache configuration
...
# Import prompt intelligence for data requirements
from prompt_intelligence_engine import PromptAnalysisResult, PromptIntent
...
class SmartDataExtractor:
    ...
    async def _fetch_table_with_smart_filtering(self, 
        ...
        # FINANCIAL DATA FRESHNESS: When use_cache=False, this entire cache check is skipped
        # ensuring we always fetch real-time data from Supabase for accuracy
        if use_cache:
            try:
                cached_data = None
        ...
        try:
            ...
            # FINANCIAL DATA FRESHNESS: When use_cache=False, this cache storage is skipped
            # ensuring no stale data gets stored that could affect future requests
            if use_cache:
                try:
                    if self.redis_available:
    ...
    def _build_smart_query(self, table: str, params: Dict[str, Any], comprehensive: bool = False, limit: Optional[int] = None):
        ...
        if currency and table in ["entity_master", "position_nav_master", "hedge_instructions", 
