...
def _resolve_dify_api_key(agent_id: Optional[str], override_api_key: Optional[str]) -> str:
    """Resolve Dify API key by priority: explicit override -> per-agent env -> default env -> legacy fallback."""
    if override_api_key:
    ...
    if agent_id and agent_id in env_map:
        api = os.getenv(env_map[agent_id])
        if api:
            return api

    api = os.getenv("DIFY_API_KEY")
    if api:
        return api

    logger.warning("DIFY_API_KEY not set; using legacy fallback key (please set env variable)")
    return "app-juJAFQ9a8QAghx5tACyTvqqG"

def get_dify_headers(agent_id: Optional[str] = None, override_api_key: Optional[str] = None):
    """Build headers with resolved API key (supports per-request override)."""
    api_key = _resolve_dify_api_key(agent_id, override_api_key)
...
def get_dify_api_url(agent_id: Optional[str] = None) -> str:
    ...
    base = None
    # Allow per-agent base URL overrides if provided
    per_agent_url_env = {
        "allocation": "DIFY_API_URL_ALLOCATION",
        "booking": "DIFY_API_URL_BOOKING",
        "analytics": "DIFY_API_URL_ANALYTICS",
        "config": "DIFY_API_URL_CONFIG",
    }
    if agent_id and agent_id in per_agent_url_env:
        base = os.getenv(per_agent_url_env[agent_id])
    if not base:
...
async def stream_from_dify(dify_payload: Dict[str, Any], agent_id: Optional[str] = None, agent_api_key: Optional[str] = None) -> StreamingResponse:
    ...
    async def generate_stream():
        try:
            if HTTPX_AVAILABLE:
                async with httpx.AsyncClient(timeout=300.0) as client:
                    async with client.stream(
                        ...
                        DIFY_API_URL,
                        json=dify_payload,
                        headers=get_dify_headers(agent_id, agent_api_key)
                    ) as response:
                        if response.status_code != 200:
                            raw = await response.aread()
                            err_str = raw.decode(errors='ignore')
                            safe_json = json.dumps({"error": "Dify API error", "details": err_str})
            ...
            safe_json = json.dumps({"error": "Streaming error", "details": str(e)})
            yield f"data: {safe_json}\n\n"
...
async def universal_prompt_processor(request: FlexiblePromptRequest):
    ...
    try:
        ...
        if mcp_mode:
            ...
            if getattr(request, "instruction_id", None):
                inputs_payload["instruction_id"] = request.instruction_id
            if getattr(request, "msg_uid", None):
                inputs_payload["msg_uid"] = request.msg_uid
            ...
            # Allow frontend to pass a per-app Dify key override
            agent_key = getattr(request, "agent_api_key", None)
            return await stream_from_dify(dify_payload, agent, agent_key)
        ...
        if getattr(request, "instruction_id", None):
            legacy_inputs["instruction_id"] = request.instruction_id
        if getattr(request, "msg_uid", None):
            legacy_inputs["msg_uid"] = request.msg_uid
